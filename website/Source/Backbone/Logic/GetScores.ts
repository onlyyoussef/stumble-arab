...
export async function GetScores(
  ...
  try {
    ...
    // ✅ FIX: نجلب فقط المستخدمين الموجودين في المباريات بدل كل المستخدمين
    const MatchUserIds = new Set<string>();
    for (const m of AllMatches) {
      for (const u of m.users || []) {
        if (u["@user-id"]) MatchUserIds.add(u["@user-id"]);
      }
    }
    ...
    if (IsFinalPhase && AllLastRoundClosed && LastRoundMatches.length > 0) {
      ...
      // ─── Hall of Fame + All-Time Leaderboard بعد انتهاء البطولة ─────────
      if (ShouldCount) {
        const tourDoc = await Tournament.findOne({ TournamentId: TournamentIdStr })
          .select("TournamentName TournamentColor TournamentImage")
          .lean();
        SendHallOfFame({
          tournamentId:    TournamentIdStr,
          tournamentName:  (tourDoc as any)?.TournamentName || TournamentIdStr,
          tournamentColor: (tourDoc as any)?.TournamentColor,
          tournamentImage: (tourDoc as any)?.TournamentImage,
        }).catch(() => {});
