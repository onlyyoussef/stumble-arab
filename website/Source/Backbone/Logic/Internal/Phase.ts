...
export async function CreateOrAssignMatch(User: IBackboneUser, Tournament: ITournament): Promise<IMatch | null> {
  ...
  const Task = (async () => {
    try {
      ...
      // FIXED: MaxTeams should be 2 (two teams per match), not MaxPlayersPerMatch
      // MaxPlayersPerMatch is the number of players (e.g., 4 in 2v2)
      // MaxTeams is always 2 (Team A vs Team B)
      const MaxTeams = 2;
      ...
      // FIXED: MinTeams should be 2 (need 2 teams to start), not MinPlayersPerMatch
      // MinPlayersPerMatch is the minimum player count (e.g., 4 in 2v2)
      // MinTeams is always 2 (need both teams present to start match)
      const MinTeams = 2;
    ...
    if (AllDone) {
      ...
      if (Sorted.length > 0) {
        ...
        if (TourWinners.length > 0) {
          ...
          // ─── تحديث TournamentsWon للفائزين ──────────────────────────────
          const WinnerIds = Array.from(WinTeam.Members);
          await BackboneUser.updateMany(
            { UserId: { $in: WinnerIds } },
          ...
          const PlaceOps = WinnerIds.map((Id) => ({
            updateOne: {
              filter: { UserId: Id },
          ...
          if (PlaceOps.length > 0) {
            await BackboneUser.bulkWrite(PlaceOps, { ordered: false });
          }
