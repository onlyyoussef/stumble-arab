...
async function ProcessExpiredMatch(ExpiredMatch: any, Tournament: ITournament, PhaseType: string): Promise<void> {
  ...
  } else if (ExpiredMatch.roundid === 1) {
    // ─── Round 1: الـ deadline = PhaseStart + 1.5 دقيقة فقط ─────────────
    // ما في SubtractedTime، فالـ MatchStartDeadline = الـ deadline مباشرة
    MatchStartDeadline = Deadline;
  } else {
  ...
  if (
    ...
    if (WinnerUserIds.length > 0) {
      ...
      await BackboneUser.updateOne(
        { UserId: WinnerUserIds[0], [`Tournaments.${ExpiredMatch.tournamentid}`]: { $exists: true } },
        { $set: { [`Tournaments.${ExpiredMatch.tournamentid}.UserMatch`]: MatchSnapshot } }
      );
...
export async function StartLoop() {
  ...
  // نستخدم setInterval بدل while(true) — لا يبلوك الـ event loop
