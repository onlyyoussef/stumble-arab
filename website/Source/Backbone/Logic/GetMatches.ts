...
import { SendHallOfFame, SendAllTimeLeaderboard } from "../../Handlers/HallOfFame";
...
// ✅ FIX: ProcessedMatches تُنظَّف دائماً حتى عند الخطأ (في finally)
...
export function GetMatchDeadline(
  ...
  // ─── Round 1: الـ deadline = PhaseStart + 1.5 دقيقة فقط ─────────────────
  // ما في SubtractedTime في الـ round 1، فنرجع الـ deadline مباشرة
  if (CurrentMatch.roundid === 1) {
    return Deadline;
  }
...
async function GenerateSingleElimination(
  ...
  for (let Round = 1; Round <= TotalRounds; Round++) {
    ...
    for (let MatchNum = 1; MatchNum <= MatchCount; MatchNum++) {
      ...
      const CheckInTime = 1.5 * 60 * 1000; // دقيقة ونص

      if (Round === 1) {
        // ─── Round 1: الـ deadline = PhaseStart + 1.5 دقيقة فقط ──────────────
        // الـ auto-win يصير بعد دقيقة ونص بالضبط من بداية البطولة
        Deadline = new Date(PhaseStart.getTime() + CheckInTime);
      } else {
  ...
  for (let Round = 1; Round <= TotalRounds; Round++) {
    ...
    for (let MatchNum = 0; MatchNum < MatchCount; MatchNum++) {
      ...
      if (Status === TournamentMatchStatus.GameReady) {
        ...
        if (Round === 1) {
          // ─── Round 1: دقيقة ونص فقط من بداية الـ phase ──────────────────
          Deadline = new Date(PhaseStart.getTime() + CheckInTime);
        } else {
          const BaseTime = new Date(PhaseStart.getTime() + Config.MaxLength * 60 * 1000 * (Round - 1));
          Deadline = new Date(BaseTime.getTime() + CheckInTime + SubtractedTime);
        }
      } else {
...
async function QualifyFromBracket(User: IBackboneUser, Tournament: ITournament): Promise<void> {
  ...
  if (IsLastPhase && IsLastRound) {
    ...
    if (Winners.length > 0) {
      ...
      // ─── تحديث TournamentsWon لكل الفائزين ────────────────────────────
      ...
      // ─── تحديث FinalPlace = 1 للفائزين ────────────────────────────────
      const TourId = Tournament.TournamentId.toString();
      const PlaceUpdates = WinnerIds.map((Id) => ({
        updateOne: {
          filter: { UserId: Id },
          update: { $set: { [`Tournaments.${TourId}.FinalPlace`]: 1 } },
        },
      }));
      if (PlaceUpdates.length > 0) {
        await BackboneUser.bulkWrite(PlaceUpdates, { ordered: false });
      }
  ...
  if (QualifiedUpdates.length > 0) {
    ...
    console.error(`[QualifyFromBracket] Error processing match ${MatchId}:`, err);
    throw err;
  } finally {
    // ✅ FIX: تنظيف ProcessedMatches دائماً حتى عند الخطأ
