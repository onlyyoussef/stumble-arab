...
export function GetProperties(DatabaseTournament: ITournament) {
  ...
  // ─── No Emotes (0): نعطّل كل الـ emotes الموجودة في الـ enum ────────────────
  // بدل توليد 1-255 نستخدم القائمة الحقيقية من الـ Emotes enum
  if (DisabledEmotes.includes(0)) {
    DisabledEmotes = Object.keys(Emotes)
      .filter((k) => isNaN(Number(k)))                          // الأسماء فقط
      .map((k) => Emotes[k as keyof typeof Emotes] as number)
      .filter((id) => id > 0);                                  // الأرقام الموجبة فقط (تستثني الـ presets السالبة)
  }
