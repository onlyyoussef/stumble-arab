...
import { UpdateWebhookSignupCount } from "../../Handlers/Database";

const App = Router();

// ─── Rate limiter: 20 محاولة كل دقيقة لكل IP ─────────────────────────────────
...
App.post(
  ...
  async (req, res) => {
    ...
    // ─── تحديث رسالة الـ webhook بالعداد الجديد (بدون انتظار) ────────────────
    UpdateWebhookSignupCount(TournamentId).catch(() => {});
