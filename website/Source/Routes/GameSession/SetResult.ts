...
App.post("/gameSessionSetResult", async (Req, Res) => {
  try {
    ...
    // Build the finished match snapshot to store in UserMatch for each user
    const FinishedMatchSnapshot = {
      ...
      status: TournamentMatchStatus.GameFinished,
      tournamentid: FoundMatch.tournamentid,
      users: FoundMatch.users,
    };

    // Sync UserMatch for all users in the match so Qualify() has fresh data to work with
    await BackboneUser.updateMany(
      { UserId: { $in: AllMatchUserIds }, [`Tournaments.${FoundMatch.tournamentid}`]: { $exists: true } },
      { $set: { [`Tournaments.${FoundMatch.tournamentid}.UserMatch`]: FinishedMatchSnapshot } }
    );
    ...
    if (WinningUsers.length > 0) {
      // Only qualify once per winning team to avoid double-processing the same match
      const ProcessedTeams = new Set<string>();
      for (const Winner of WinningUsers) {
        const WinnerMatchUser = FoundMatch.users.find((U) => U["@user-id"] === Winner.UserId);
        const WinnerTeamId = WinnerMatchUser?.["@team-id"];
        if (WinnerTeamId && ProcessedTeams.has(WinnerTeamId)) continue;
        if (WinnerTeamId) ProcessedTeams.add(WinnerTeamId);
