...
class UserModel {
  ...
  let ipCountry = platformData.Country || 'US';
  let ipRegion = 'NA';
  ...
  try {
    ...
    ipCountry = platformData.Country || 'US';
    ipRegion = 'NA';
  }
  ...
  try {
    ...
    if (req.path == "/user/login" || req.path == "/user/config" || req.path == "/photon/get" || req.path == "/onlinecheck" || req.path.startsWith("/shared/") || req.path == "/api/v1/ping" || req.path == "/version/get" || req.path.startsWith("/admin/") || req.path == "/auth") {
      return next();
    }
