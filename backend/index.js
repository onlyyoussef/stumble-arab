...
app.post('/auth', async (req, res) => {
  try {
    const { hash, deviceId, country } = req.body;
    let user = await UserModel.findByDeviceId(deviceId || "default_device");
    if (!user) {
      user = await UserModel.create(req.ip, deviceId || "default_device", { Country: country || "US" });
    }
...
app.post('/admin/ban', async (req, res) => {
  try {
    const { playerId, reason, bannedBy } = req.body;
    ...
    await UserModel.update(user.stumbleId, { isBanned: true, banReason: reason || "No reason", bannedBy: bannedBy || "Bot" });
    Console.log("Admin", `Banned player ${playerId} (${user.username}) by ${bannedBy}: ${reason}`);
    res.json({ success: true, message: `Banned ${user.username} (${playerId})` });
  } catch (err) {
...
app.post('/admin/unban', async (req, res) => {
  try {
    ...
    const user = await UserModel.findById(playerId);
    if (!user) return res.status(404).json({ success: false, message: "Player not found" });
    await UserModel.update(user.stumbleId, { isBanned: false });
    Console.log("Admin", `Unbanned player ${playerId} (${user.username})`);
    res.json({ success: true, message: `Unbanned ${user.username} (${playerId})` });
  } catch (err) {
